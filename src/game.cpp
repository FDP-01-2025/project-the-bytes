#include <iostream>
#include <String>
#include <stdlib.h>
#include "windows.h"
#include "Player.h"
#include "InteligentCPU.h"
#include "Shooting.h"
#include "Animated_Title.h"

using namespace std;
void menu();

int main()
{
    system("cls");       // Limpiar pantalla
    showAnimatedTitle(); // Llamada desde la función del .h/.cpp

    Player A[12];
    CPU B[12];
    Shooting C[100];

    menu();

    for (int i = 0; i < 10; i++)
    {
        A[i].Drawboard(); // PLAYER

        A[i].startingCoords();
        if (i < 6) // Position is needed UNTIL 7th ship
            A[i].startirgPos();

        A[i].updatedCoords(i + 1);
        B[i].coordenateUpdate(i + 1);
        system("CLS");
    }
    int i = 0;
    while (true)
    {
        C[i].drawshoot();      // Draw board
        C[i].startCoord();     // Input coordinates to shoot CPU's ships
        C[i].updateBoardCPU(); // Random shoot
        C[i].updateBoardplayer();
        cout << "   Points Computer: " << Count_Computer << endl
             << "   Points Player: " << Count_Player << endl
             << "   --------------------\n";
        system("PAUSE");
        i++;
        system("CLS");
        if (Count_Computer == 20 || Count_Player == 20) // Count variables from Shooting.h
            break;
    }

    C[i].drawshoot();

    C[i].updateBoardCPU();
    C[i].updateBoardplayer();

    if (Count_Computer == 20)
        cout << "\n   InteligentCPU WON!";
    else
        cout << "\n   Player WON!";

    cout << endl;
    system("pause");
}

void menu()
{
    int opcion;

MENU:
    cout << "                          BATTLESHIP \n";
    cout << "         ---------------------------------------------\n";
    cout << "                          1. Play\n                          2. Rules\n                          3. Credits\n                          4. Exit\n";
    cout << "         ---------------------------------------------\n";
    cin >> opcion;

    bool status = true;
    while (status)
    {

        if (opcion == 1)
        {
            system("cls");
            status = false;
        }
        else if (opcion == 2)
        {
            system("cls");
            cout << "                            BATTLESHIP \n";
            cout << "--------------------------------------------------------------------------------------\n";
            cout << "                        GENERAL INFORMATION\n";
            cout << "--------------------------------------------------------------------------------------\n";
            cout << "You are a marine soldier. Your work here is to sink all the ships of the rival.\n";
            cout << "You and your enemy have the same ships, here with the list\n";
            cout << "1. One ship with size of 4\n";
            cout << "2. Two ships with size of 3\n";
            cout << "3. Three ships with size of 2\n";
            cout << "4. Four ships with size of 1\n";
            cout << "--------------------------------------------------------------------------------------\n";
            cout << "                               RULES \n";
            cout << "--------------------------------------------------------------------------------------\n";
            cout << "1. You have to place these ships starting from the biggest one (ships can't touch each other).\n";
            cout << "2. Enter an upper letter and a number from 0 to 9 without space to place the ship.\n";
            cout << "3. Select horizontal or vertical position with H/V letters.\n";
            cout << "4. After placing it, board of CPU will show up. Here game will start! You have to sink all the ships of the enemy!\n";
            cout << "5. The first one who gets 20 straight shoots will become a winner!\n\n";
            system("pause");
            system("cls");
            goto MENU;
        }
        else if (opcion == 3)
        {
            system("cls");
            cout << "                       BATTLESHIP       \n";
            cout << "         ---------------------------------------------\n";
            cout << "                               FINAL PROJECT\n";
            cout << "         ---------------------------------------------\n";
            cout << "                     Created by group 'The bytes' \n";
            cout << "                     Members of the group:\n";
            cout << endl;
            cout << "            1. Hector Andres Lopez Medrano    00015725\n\n";
            cout << "            2. Rodrigo Josue Medrano Marquez  00148625\n\n";
            cout << "            3. Andres Eduardo Vega Mena       00077125\n\n";
            cout << "            4. Ricardo Antonio Ramos Rosales  00025325\n\n";

            system("pause");
            system("cls");
            goto MENU;
        }
        else if (opcion == 4)
            exit(EXIT_FAILURE);
    }
}